...
| **词组快打** | 62 万词组（雾凇 rime-ice，含成语/现代词），**每字打前 4 笔**即出词（不足 4 笔打全部） |
...
- **单字/词组彻底分流**（v1.1）：单字走前缀联想（打 3 笔出常用字），词组走精确匹配（打满编码才出现）——词组不参与短前缀竞争，常用单字不被挤出第一页
- **词组编码**：每字前 4 笔拼接（只保留简码，不存全码），4 笔同码少、统一规则无选择负担；2 字词 = 8 笔码（5⁸≈39 万槽位，62 万词下几乎不撞码）
- **候选排序**：词组权重 ×10、55 个高频单字 ×100、雾凇词频（比 jieba 更现代合理）、用户词典动态调频
