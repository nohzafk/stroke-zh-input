...
| 文件 | 说明 |
|---|---|
| `scripts/make_predict_data_ice.py` | 生成预测文本数据（雾凇词库，**纯后缀 value**，GB2312 过滤，668892 条） |
| `predict.db` | 最终预测库（9.1MB，已打包进 stroke_zh.zip） |
...
```sh
# 1. iSH: 生成预测数据 (雾凇词库后缀拆分, GB2312 过滤)
python3 -B scripts/make_predict_data_ice.py > /tmp/predict_data.txt

# 2. 传到 Mac
scp /tmp/predict_data.txt randall@192.168.0.243:/tmp/predict_data.txt

# 3. Mac: 生成 predict.db (需要 librime 动态库: brew install librime)
...
- **数据过滤**：make_predict_data_ice.py 已过滤生僻字（GB2312 常用字 + 雾凇词频≥100）——预测候选干净
- **后缀 value 铁律**：predict.db 的 value 必须是**纯后缀**（词「下一页」→ key=下 value=一页、key=下一 value=页），librime-predict 候选是 0 宽度后缀、点击只上屏 value 本身——若 value 存完整词，点击会与已上屏字拼接成重复（下+下面=下下面）。改脚本后务必用 Darts+marisa 解析器直接验证 dist 里的 predict.db
