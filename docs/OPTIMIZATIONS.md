...
| **v1.4.0** | **视觉块输入**：词组编码改每字**自然码连续串**（我们=`phshpsnsz` 我4+们5）；词频换 jieba；简码扩 1~7 笔（修"多打一笔更差"倒挂）；新增 `typing_test.py` 打字效果回归。**升级须清空用户词典** | 见 docs/DESIGN.md §8 |

**打词组规则（v1.4 起）**：每个字按**自然码**打（视觉块停笔，3~5 笔），连续打满编码（我们 = 我4+们5 = 9 笔）精确上屏；不打满靠补全（第 6~8 笔进第一页）。4-5 笔词组默认不收（librime remaining 主键机制），**A 方案放行超高频词**（第一/一个/一些/一般/一定/一下 等 22 个，jieba 词频≥1 万）；6-7 笔词组权重 ×0.001（放行词保留全权重）；8+ 笔正常。
**单字规则（v1.3 起）**：1~7 笔都是简码精确匹配（v1.4 扩到 7 笔）→ 任意笔数越高频字越靠前；命中率对笔数单调递增。
...
| 迭代 | 英文键盘行4：`123 ， 空格 。 ？ #+= 中英 回车` → **去逗号/问号**、`#+=` 移最左：`#+= 123 空格 中英 回车`（宽度/字号与中文底部行逐键对齐，切换不跳动） |
| 迭代 | 符号键盘：自定义 symbolic 失败（元书不支持/空白）→ **用元书内置** |
| 迭代 | **自定义 symbolic 键盘**（v2.0）：内置符号键盘被取代——3 行标点（中文标点 20 + 英文半角 10，仿系统 123 键盘布局）+ 底部行 `🔒 ABC 空格 返回 回车` |
| 迭代 | **标点锁定键**（🔒 `symbolicKeyboardLockStateToggle` + `$symbolicKeyboardLockState` 条件样式）：锁定后连打标点不退回主键盘，再点解锁；未锁定时点标点自动回主键盘（与系统行为一致）。内置 symbolic 键盘「按一下退出」痛点解决 |
| 迭代 | 符号键盘返回：`ABC`（returnPrimaryKeyboard 回主键盘）+ `返回`（returnLastKeyboard 回上一键盘，从数字键盘进入时回数字） |
| 迭代 | 「？」键 → **清除**（`#重输`，打错一键清空） |
...
**打包铁律**（防修 A 破坏 B）：完整规程见 **docs/SKIN_MODIFICATION.md**（改皮肤前必读）；
改完必跑 `python3 scripts/verify_skin.py`（pre-commit hook 自动拦截非预期变化）。
一句话版：`cskin/` 源只对应笔画拼音；笔画增强无源必须解包旧包增量更新；禁止全量覆盖。
...
| `scripts/typing_test.py` | **打字效果回归**（44 用例：词组连打/单字自然码/少打补全/4-5 笔排除断言，改编码后必跑，报告输出 `docs/typing_report.md`） |
